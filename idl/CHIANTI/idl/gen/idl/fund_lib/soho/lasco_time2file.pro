...
;      lasc2=lasco_time2file(time0,time1,/c2,/nascom) ; use nascom/qkl
;
;  History:
...
;    18-feb-2013 - S.L.Freeland - add /nascom option (qkl swpc_cat NRT CME analysis for example)
;    19-feb-2013 - S.L.Freeland - assure chronological output & C3 tweak
;
;  Restrictions:
...
nascom=keyword_set(nascom) ; zeus.nascom.nasa.gov , mostly QKL
quicklook=keyword_set(quicklook) ; quicklook NRL 

case 1 of 
   nascom: urlp='ftp://zeus.nascom.nasa.gov/
...
case 1 of 
   nascom: suburls=urlp+'/qkl/lasco/quicklook/' + 'level_' + slev + '/' + subdirs + '/' + stel+'/'   
   quicklook: suburls=urlp+ '/ql/' + 'level_' + slev + '/' + subdirs + '/' + stel+'/'
   else: suburls=urlp+'/lz_data/' + 'level_' + slev + '/' + subdirs + '/' + stel+'/'
endcase

retval=''

if nascom then begin 
   lindex=''
   for i=0,n_elements(suburls)-1 do begin 
      box_message,'Listing>> ' + suburls[i]
      sock_list,suburls[i]+ 'img_hdr.txt', out
      temp=suburls[i] + ' ' + out
      lindex=[lindex,temp]
   endfor
   ssn=where(strmatch(lindex,'*1024*Normal*',/fold_case),ncnt)
   if ncnt gt 0 then begin 
      lindex=lindex[ssn] ; normal subset
      cols=str2cols(lindex,/unal,/trim) ; 1D -> 2D
      strtab2vect,cols,urls,fnames,ddate,dtime ; 2D - nX1D
      ftimes=ddate + ' ' + dtime
      order=sort(anytim(ftimes))
      urls=urls[order]
      ftimes=ftimes[order]
      fnames=fnames[order]
      sst=where(anytim(ftimes) ge anytim(time0) and anytim(ftimes) le anytim(time1),tcnt)
      if tcnt gt 0 then begin 
         retval=urls[sst] + fnames[sst]
         ftimes=ftimes[sst]
      endif else box_message,'No images for '+ stel + ' in your time range@ ' + urlp
   endif
endif else begin 
for i=0,n_elements(suburls)-1 do begin 
...
endelse
endelse ; end of not-NASCOM (e.g. nrl original)
