...
;-- check for overriding keywords
   ...
   if (1-vstruct_in) then begin
    ...
    need_l0=1-is_number(l0)
    if need_rad or need_b0 or need_l0 then begin
     dprint,'calling pb0r...'
...
;   
   ...
   answer = FLTARR(2, MAX([N_ELEMENTS(colat), N_ELEMENTS(lon),N_ELEMENTS(robs)]),/nozero)
   xcoord = fltarr(max([n_elements(colat),n_elements(lon),n_elements(robs)]),/nozero)
;
