...
pro get_fits_par,header,xcen,ycen,dx,dy,err=err,time=time,stc=stc,nx=nx,$                
                ...
                dur=dur,soho=soho,_extra=extra                                                        
...
keys=['spacec','obsrvtry','observa','telesc','instrume','detect','wavelnth','wave_len']
id=strarr(nimg)
for i=0,nimg-1 do begin
 for j=0,n_elements(keys)-1 do begin
  if have_tag(stc[i],keys[j],k) then begin
   item=trim(stc[i].(k[0]))
   if is_string(item) then if ~stregex(id[i],item,/bool,/fold) then id[i]=id[i]+' '+item                 
  endif                     
 endfor
endfor
