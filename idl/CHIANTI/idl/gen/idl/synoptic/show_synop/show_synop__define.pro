...
;info={tstart:0l,tend:0l,subint:0l,main:0l,lfont:lfont,bfont:bfont,brow:0l,srow:0l,$
;      site_drop:0l,$
;      timer:0l,$
;      slist:0l,slist2:0l,flist:0l,cur_sel:'',cur_fsel:'',$
;      site_base:0l,sbuttons:[0l,0l,0l],$
;      config_file:config_file,dtext:0l,$
;      tbase:0l,hbase:0l,group:0l,messenger:0l,clear_time:systime(/sec),$
;      wait_time:12*3600.}
...
; tstart = times[0]
; tend = times[1]
; if ~valid_time(tstart,err1) then begin
...
; if ~valid_time(tend,err2) then begin
...
pro show_synop::setprop,info=info,sort_mode=sort_mode,err=err,$
               save_time=save_time,_extra=extra,clobber=clobber,$
               site=site,plotman=plotman,up_prep=up_prep,$
...
;-- calendar buttons
 ...
 if bname eq 'times' then begin
  widget_control, info.wtimes, get_value=times
;  times = anytim2utc(times,/vms)
  self->setprop,tstart=times[0],tend=times[1]
;  print,'setting tstart,tend to ' + times[0] + ' ' + times[1]
 endif
 
; if (event.id eq info.tstart) or (event.id eq info.tend) then begin
;  ostart=self->getprop(/tstart)
;  oend=self->getprop(/tend)
;  cstart=anytim2tai(ostart)
;  cend=anytim2tai(oend)
;  if (event.id eq info.tstart) then begin
;   widget_control,event.id,get_value=tstart
;   if ~valid_time(tstart,err=err) then begin
;    xack,err
;    widget_control,event.id,set_value=anytim2utc(ostart,/vms)
;    return
;   endif
;   if anytim2tai(tstart) ge cend then begin
;    xack,'Start time must be less than end time.'
;    widget_control,event.id,set_value=anytim2utc(ostart,/vms)
;    return
;   endif
;   tstart=anytim2utc(tstart,/vms)
;   widget_control,event.id,set_value=tstart
;   self->setprop,tstart=tstart
;   return
;  endif
; 
;  if (event.id eq info.tend) then begin
;   widget_control,event.id,get_value=tend
;   if ~valid_time(tend,err=err) then begin
;    xack,err
;    widget_control,event.id,set_value=anytim2utc(oend,/vms)
;    return
;   endif
;   if anytim2tai(tend) le cstart then begin
;    xack,'End time must be greater than start time.'
;    widget_control,event.id,set_value=anytim2utc(oend,/vms)
;    return
;   endif
;   tend=anytim2utc(tend,/vms)
;   widget_control,event.id,set_value=tend
;   self->setprop,tend=tend
;   return
;  endif
;  return
; endif
;
; if (bname eq 'tstart') or (bname eq 'tend') then begin
;  cstart=anytim2tai(self->getprop(/tstart))
;  cend=anytim2tai(self->getprop(/tend))
;  if (bname eq 'tstart') then begin
;   odate=anytim2utc(self->getprop(/tstart),/vms,/date)
;   otime=anytim2utc(self->getprop(/tstart),/vms,/time)
;   xcalendar,odate,cancel=cancel
;   if cancel then return
;   odate=anytim2utc(odate,/vms,/date)
;   cdate=odate+' '+otime
;   if anytim2tai(cdate) ge cend then begin
;    xack,'Start time must be less than end time.'
;    return
;   endif
;   widget_control,info.tstart,set_value=cdate
;   self->setprop,tstart=cdate
;  endif
;  if (bname eq 'tend') then begin
;   odate=anytim2utc(self->getprop(/tend),/vms,/date)
;   otime=anytim2utc(self->getprop(/tend),/vms,/time)
;   xcalendar,odate,cancel=cancel
;   if cancel then return
;   odate=anytim2utc(odate,/vms,/date)
;   cdate=odate+' '+otime
;   if anytim2tai(cdate) le cstart then begin
;    xack,'End time must be greater than start time.'
;    return
;   endif
;   widget_control,info.tend,set_value=cdate
;   self->setprop,tend=cdate
;  endif
; endif
...
;  if ~self->check_times() then return
...
;-- check if sending to Prepserver
    ...
    xtext,'Reading '+rfile+'...',wbase=tbase,/just_reg,/center,$
         group=info.main
    xhour
...
;-- use data's internal plot method if not using PLOTMAN
 ...
 if ~use_plotman then begin
  if exist(plot_obj) then plot_obj -> plot, err=err else data->plot,err=err
...
;-- use data object plotman method if available
 ...
 if have_method(data,'plotman') then begin
  data->plotman,plotman=!show_synop.plotman_obj,/noclone,nodup=nodup,replace=replace, /use_colors
  return
 endif
...
;-- check if site requires time rounding or shouldn't be cached
...
for i=0,rcount-1 do begin
 ...
 if do_down then begin
  sock_copy,rfiles[i],out_dir=ldir,_extra=extra,err=err,local_file=nfile,$
            cancel=cancel,clobber=clobber,progress=(i eq 0),$
...
pro show_synop::rsearch,files,count=count,err=err,_ref_extra=extra
...
self={show_synop,save_time:0.d,sort_mode:0,ptr:ptr_new(),clobber:0b,$
      site:'',plotman:0,down_prep:0b,up_prep:0b,prep_widg: 0b,$
