...
acro=['xrs','(^aia|_AIA_)','^(EVL|EVS)','\.les','^glg','^(prepped_)?eis','^mdi','^cont','^fdmg','^(prepped_)?xrt','^kanz','^eit','^(prepped_)?efr',$
      ...
      '^(prepped_)?[^ ]+(c1A\.|c1B\.)','^(prepped_)?[^ ]+(c2A\.|c2B\.)']

class=['messenger','aia','eve','soxs','fermi_gbm','eis','mdi','mdi','mdi','xrt2','kanz','eit','eit','eit',$
       'trace','trace',$
       'spirit','bbso','kpno','rstn','ethz','hxrs','euvi',$
       'ovsa_ltc','osra','synop_spex','nobeyama','nancay','meudon','callisto','cor1','cor2']
...
if is_blank(fclass) then begin
 dclass=get_fits_det(file,/quiet)
 if valid_class(dclass) then fclass=dclass
endif
