...
pro secchi::read,file,data,_extra=extra,err=err
...
;-- check if RICE-compressed
 ...
 rice=0b
 if is_rice_comp(cfile[i]) then begin
  rfile=rice_decomp(cfile[i],err=err)
  if is_string(err) then begin
   message,err,/info
   continue
  endif
  rice=1b
 endif else rfile=cfile[i]

 valid=self->is_valid(rfile,level=level,_extra=extra)
...
;-- intercept outsize keyword
 ...
 if have_cal and have_path and (level eq 0) then begin
  secchi_prep,rfile,index,data,_extra=extra
 endif else begin
  if (level eq 0) then begin
   if ~have_cal then xack,err1,/suppress
   if ~have_path then xack,err2,/suppress
   message,'Skipping prepping.',/info
  endif
  self->fits::read,rfile,data,_extra=extra,index=index
 endelse
...
case 1 of
 ...
 resize: begin
  message,'Resizing to ['+trim(outsize[0])+','+trim(outsize[1])+']...',/info
  map=drot_map(map,outsize=outsize)
...
if (nint(index.crota) mod 360.) eq 0 then begin
 message,'Map already roll-corrected.',/info
 return
endif

;-- roll correct

message,'Correcting for spacecraft roll...',/info

map=drot_map(map,roll=0.,_extra=extra,/same_center)
index.crota=0.
self->update_pc,index,map
...
message,'Correcting to Earth-view...',/info
...
if keyword_set(euvi) and detector ne 'EUVI' then begin
 if verbose then message,'Input file is not EUVI',/info & return,0b
endif

if keyword_set(cor1) and detector ne 'COR1' then begin
 if verbose then message,'Input file is not COR1',/info & return,0b
endif

if keyword_set(cor2) and detector ne 'COR2' then begin
 if verbose then message,'Input file not COR2',/info & return,0b
endif
