...
; Name        : RDWRT_BUFF
;
; Purpose     : read & write unformatted data from socket to file
;
...
;
; Keywords    : MAXSIZE = max size in bytes to read/write 
...
;               DATA = returned data as byte array
;               ASCII = set to convert to ASCII
;
...
pro rdwrt_buff,ilun,olun,maxsize=maxsize,buffsize=buffsize,counts=counts,err=err,$
               _extra=extra,omessage=omessage,chunked=chunked,data=output,ascii=ascii,$
...
writing=~arg_present(output)
if writing then begin
 if ~is_number(olun) then return
 if ~(fstat(olun)).open then return
endif
...
repeat begin
 ...
 if chunked then begin
  ...
  if ~exist(old_bsize) then data=bytarr(bsize,/nozero) else begin
