...
;               returns the string 'None'.  If your format for non-ut intervals isn't wide
;               enough, a default format of g15.4 will be used (just for those items).
;
...
;	31-Jan-2011, Kim.  When format width isn't sufficient for numbers (and gives ***s),
;	             reformat just the items with stars with g15.4.
;
...
if keyword_set(ut) then begin
	...
	q = where (strpos(i1,'*') gt -1, cstar)
	if cstar gt 0 then i1[q] = strtrim (fstring (int[0,ind[q]], format='(g15.4)'), 2)
	i2 = strtrim (fstring (int[1,ind], format=format), 2)
	q = where (strpos(i2,'*') gt -1, cstar)
  if cstar gt 0 then i2[q] = strtrim (fstring (int[1,ind[q]], format='(g15.4)'), 2)
