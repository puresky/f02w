...
widget_control, state.w_ulabel, set_value=*state.ulabel
widget_control, state.w_ulabel_lev, set_value=*state.ulabel_lev
...
struct = str_subset(pc, $
	['drotate_image', 'overlay_roll', 'smooth_image', $
	 ...
	 'overlay_ulabel', 'overlay_ulabel_lev'] )
...
w_copy_levs = cw_bgroup (w_base3a, 'Copy levels to all contours', uvalue='copy', /nonexclusive)
