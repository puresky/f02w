...
if sel_overlay ne -1 then begin
    ...
    if sel_overlay eq 0 then begin
      ...
      ; Otherwise, set overlay to panel selected. Set user label to the panel
      ; description after removing the 'Cube' and current time parts of the string. Leave
      ; ulabel_lev (option to append contour levels) alone.
      overlay_panel[overlay_num] = ostate.image_panel_descs[sel_overlay]
...
w_axesandsurf = widget_base(w_box, /row, /frame, space=10)

w_axes_base = widget_base (w_axesandsurf, /row, /frame)

w_xyz_base = widget_base (w_axes_base, /column, space=0, ypad=0)

tmp_base = widget_base (w_xyz_base, /row)
...
w_surf_base = widget_base (w_axesandsurf, /column, /frame, /base_align_center)
...
w_opt_base = widget_base (w_box, /row, /frame, space=3, /align_center)
...
w_opt_base2= widget_base (w_opt_base, /column, space=5)
...
w_grid_thick = plotman_defaults_wrapper(defaults, 'cw_field', tmp_base, $
					...
					uname='grid_thickness')

w_opt_base4 = widget_base (w_opt_base, /column, space=5)

tmp_base = widget_base (w_opt_base4, /row)
