...
if not do_image and ov_squish then begin
	...
	if !p.multi[2] ne 0 then begin
		...
		if !p.multi[0] ne 1 then args = args + ", xtickname=strarr(30)+' ', /nolabel, xtitle=''"  ; not bottom plots
	endif
endif
...
; not necessary?
; oops, this is necessary for stacked PS plots.  plotman_create_files_event
; only switches white to black for the first plot, so need this here for others.
if !d.name eq 'PS' and exist(dim1_colors) then begin
...
;					ov_self = 0
					...
					ov_save = str_subset(self.plot_control, $
						['overlay_roll', 'smooth_image', $
						 ...
						 'overlay_ulabel', 'overlay_ulabel_lev', 'no_timestamp'] )
...
if not exist(data) then data = (convert_coord  (x, y, /device,/to_data))[0:1]; just x,y not z
