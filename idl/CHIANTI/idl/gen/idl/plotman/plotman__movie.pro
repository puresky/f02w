...
if nframe gt 1 then begin   ; More than 1 panel, movie over different panels
    ...
    frames = fltarr(dim[0], dim[1], nframe)
  ...
  for ii=0, nframe-1 do begin
    ...
    if panel_plot then begin
      ...
      if norm_each then frames[0,0,ii] = congrid(bytscl(im), dim[0], dim[1]) else $
        frames[0,0,ii] = congrid(im, dim[0], dim[1])
    endelse
  endfor
  if not norm_each then frames = bytscl(frames)
...
if ~java and xregistered('XInterAnimate2') then begin
