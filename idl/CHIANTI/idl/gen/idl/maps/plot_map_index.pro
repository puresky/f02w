...
;     X and Y). 
;     ROLL = angle (degrees) to rotate input map (positive clockwise).
;
; Restrictions:
...
pro plot_map_index,map,contour=cont,overlay=overlay,smooth_width=smooth_width,border=border,$
 ...
 roll=roll,xshift=xshift,yshift=yshift
...
common plot_map,last_window,last_time,last_drange,last_top,last_bottom,$
       ...
       last_b0,last_l0,last_rsun,saved_map
...
if shifting or rolling then begin
 saved_map=map
 if rolling then map=rot_map(map,roll,/no_copy)
...
ang_error=''
angles=get_map_angles(map,err=ang_error)
b0=angles.b0
l0=angles.l0
rsun=angles.rsun
...
if cont then begin
 if over then begin
  ...
  dum=drot_map(map,time=last_time,trans=trans,$
   ...
   no_drotate=no_drotate,no_project=no_project,no_roll_correct=rolling,$
    _extra=extra)
 ...
 if n_elements(levels) gt 0 then begin
  ...
  if keyword_set(percent) then plevels=levels*prange[1]/100. else begin
   if dlog then begin
    ...
    if pcount eq 0 then begin
     message,'Contour levels must be greater than zero for log scale - using default set',/info
     plevels=def_levels
 ...
 if wbuff then begin
  ...
  if (comptype eq 7) and exist(back) then begin
   ...
   if have_colors then begin
    pic=mk_24bit(pic,map.red,map.green,map.blue,true=3,/no_scale,/no_copy)
...
;-- if Z-buffer, we have to physically avoid pixels outside viewport
 ...
 if since_version('5.2') and keyword_set(cbar) then begin
