...
if ~exist(xcen) then xcen=0.
if ~exist(ycen) then ycen=0
if ~exist(dx) then dx=1.
...
if ~exist(roll_angle) then roll_angle=0.
...
map=create_struct('time',stime,'id',id,'dur',dur,$
                 ...
                 'roll_angle',float(roll_angle mod 360.),$
                 'roll_center',reform(float(roll_center)))
