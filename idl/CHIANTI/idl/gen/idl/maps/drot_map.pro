...
if preserve_area or adjust_resolution then track_center=0b
...
;-- override with b0, l0, or rsun entered as keywords
 ...
 do_proj=~match_struct(pstart,pend) and ~keyword_set(no_project) 
...
;-- check if rolling
 ...
 do_roll=~keyword_set(no_roll_correct) and ((nint(roll_diff) mod 360.) ne 0.) and (abs(roll_diff) gt 1.)
...
;-- check if new roll center
 ...
 do_rcenter=0b
 if (do_roll and have_roll) then begin
...
;-- check if rebinning
 ...
 if do_resize then begin
  new_space[0]=dx*float(nx)/float(new_size[0])
  new_space[1]=dy*float(ny)/float(new_size[1])
 endif
 if n_elements(dspace) eq 2 then new_space=float(dspace)
 if n_elements(resolution) eq 1 then new_space=float([resolution,resolution])
 if n_elements(resolution) eq 2 then new_space=float(resolution)

 do_rebin=(new_space[0] ne dx) or (new_space[1] ne dy) or adjust_resolution
 ...
 if ~do_proj and ~do_drot and $
    ~do_rcenter and ~do_trans and $
    ~do_rebin and ~do_roll and $
    ~do_center and ~do_resize then begin
...
;-- flag projected limb pixels 
   ...
   if keep_limb and (ocount2 gt 0) then begin
    xlimb=xr[olimb2]
    ylimb=yr[olimb2]
   endif
...
;   yp[find]=-999999.
  ...
  temp=interp2d(pmap.data,xp,yp,xr,yr,_extra=extra,missing=0)  
...
;-- make sure all map tags are ok
 ...
 if is_struct(pend) then begin
  ...
  if have_tag(pmap,'rsun') then pmap.rsun=pend.rsun else pmap=add_tag(pmap,pend.rsun,'rsun',/no_copy) 
 endif
