...
use_sub=n_elements(sub) gt 0 or n_elements(ref_sub) gt 0  ; sub FOV?
...
;-- correct for SOHO roll
  ...
  if soho[i] and roll_correct then begin
   if have_soho_roll(stc[i]) then begin
    if verbose then message,'correcting for SOHO 180 degree roll',/info
...
;-- create map
  ...
  mapi=make_map(fdata,xc=xcen[i],yc=ycen[i],dx=dx[i],$
   ...
   err=err,roll_center=roll_center[i,*],_extra=dangles,sub=dsub,/no_copy,soho=soho[i])

  if is_string(err) then begin
   smess='nx,ny = ('+num2str(nx[i])+','+num2str(ny[i])+')'
   message,'skipping image '+num2str[i]+' -> '+smess,/info
   continue
  endif
