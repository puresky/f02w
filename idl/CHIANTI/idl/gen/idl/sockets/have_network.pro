...
function have_network,url,verbose=verbose,err=err,_extra=extra,$
         interval=interval,reset=reset,use_network=use_network
...
if verbose then message,'Trying '+test_url,/info

state=0b
hfunct='sock_response'

if since_version(6.4) then begin
 if have_proxy() or keyword_set(use_network) then hfunct='sock_header'
endif

chk=call_function(hfunct,test_url,_extra=extra,err=err,/host,code=code,location=location)
state=code eq 200
...
if count eq 1 then begin
 ...
 now=systime(/seconds)
 new_url={url:test_url,state:state,time:now,err:err}
