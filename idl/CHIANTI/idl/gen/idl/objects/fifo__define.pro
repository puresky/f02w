...
if replacing and verbose then message,'Overwriting - '+trim(id),/info
...
if replacing then begin
 ...
 if bcount eq 0 then begin
  if verbose then message,'Increasing memory size',/info
  self->boost
  pointer=self.pointer
  struct=*pointer
  ids=struct.ids
...
if (is_string(id)) then begin
 ...
 if count gt 0 then pos=chk[0] else if verbose then message,'No element match found for '+id,/info
endif else begin
 if (id gt -1) and (id lt np) then pos=id else begin
  if verbose then message,'Out of range index',/info
 endelse
endelse   
...
struct=*self.pointer
if keyword_set(verbose) then message,'Restoring - '+trim(id),/info
...
if ~ptr_valid(ptr) then begin
 message,'Invalid pointer',/info & help,ptr 
 return
endif

if ~exist(*ptr) then begin
 message,'Empty pointer',/info & help,ptr 
 return
endif
