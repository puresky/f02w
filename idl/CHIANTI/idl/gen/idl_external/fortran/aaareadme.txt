...
a MAKE.COM file and option files for VMS.  For example, under OSF one would
type

	make -f Makefile.osf

Currently supported operating systems are OSF, VMS, and SunOS.  Other operating
systems would require additional makefiles or equivalents.  CALL_EXTERNAL is
not supported under Ultrix.
...
Currently, the only routine supported are rearrange.pro and check_sum.pro,
although other routines are anticipated to be added in the future.
